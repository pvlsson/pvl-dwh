...
add_eur_amounts as (
    select
        ...
        rates.rate_to_eur,
        case
            when add_categories.withdrawal_currency = 'EUR' then add_categories.withdrawal_amount
            else rates.rate_to_eur * add_categories.withdrawal_amount
        end as withdrawal_amount_eur
    from add_categories
    left join {{ ref('stg_currency_rates') }} as rates
        on rates.currency = add_categories.withdrawal_currency
        and rates.date    = date(add_categories.transaction_datetime)
)

select *
from add_eur_amounts
